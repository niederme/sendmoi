...
- If `Auto-send` is enabled and a default recipient is already saved, it gives you 1.5 seconds to edit or switch to a saved recipient, briefly shows those choices as locked in, then sends automatically.
- While auto-send is in progress, the sheet shows an `Auto-Sending...` state with a secondary `Edit` action that stays available during that 1.5-second grace period and still cancels the in-flight auto-send attempt without changing the saved `Auto-send` preference.
...
For local manual deploys, the script prefers `~/.ssh/send_moi_deploy` automatically when present, then falls back to the legacy `~/.ssh/sendmoi_deploy` name and older shared deploy keys unless `DEPLOY_IDENTITY_FILE` is set explicitly.
